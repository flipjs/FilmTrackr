...
(function() {
	...
	describe('Cameras Controller Tests', function() {
		...
		it('CamerasController.find() should create an array with at least one Camera object fetched from XHR', inject(function(Cameras) {
			...
			CamerasController.find();
			$httpBackend.flush();

			// Test scope value
			expect(CamerasController.cameras).toEqualData(sampleCameras);
		}));

		it('CamerasController.findOne() should create an array with one Camera object fetched from XHR using a cameraId URL parameter', inject(function(Cameras) {
			...
			// Run controller functionality
			CamerasController.findOne();
			$httpBackend.flush();

			// Test scope value
			expect(CamerasController.camera).toEqualData(sampleCamera);
		}));

		it('CamerasController.create() with valid form data should send a POST request with the form input values and then locate to new object URL', inject(function(Cameras) {
			...
			// Run controller functionality
			CamerasController.create(sampleCameraPostData);
			$httpBackend.flush();
		...
		it('CamerasController.update() should update a valid Camera', inject(function(Cameras) {
			...
			CamerasController.camera = sampleCameraPutData;
		...
		xit('CamerasController.remove() should send a DELETE request with a valid cameraId and remove the Camera from the scope', inject(function(Cameras) {
			...
			CamerasController.cameras = [sampleCamera];

			// Set expected DELETE response
			$httpBackend.expectDELETE(/cameras\/([0-9a-fA-F]{24})$/).respond(204);

			// Run controller functionality
			CamerasController.remove(sampleCamera);
			$httpBackend.flush();

			// Test array after successful delete
			// expect(CamerasController.cameras.length).toBe(0);
		}));
	});
}());
